...
namespace Sledge.Rendering.DataStructures
{
    public class Box
    {
        ...
        public Box(IEnumerable<Vector3> vectors)
        {
            if (!vectors.Any())
            {
                throw new Exception("Cannot create a bounding box out of zero Vector3s.");
            }
        ...
        public bool IsEmpty()
        {
            return Math.Abs(Width) < 0.0001 && Math.Abs(Height) < 0.0001 && Math.Abs(Length) < 0.0001;
        }
        ...
        */
        ...
        /// <param name="c"></param>
        /// <returns></returns>
        public bool VectorIsInside(Vector3 c)
        {

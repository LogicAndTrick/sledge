    ...
    public class TextureAxisPerpendicularToFace : IProblemCheck
    {
        public IEnumerable<Problem> Check(MapDocument document, bool visibleOnly)
        {
            ...
            foreach (var face in faces)
            {
                ...
                if (Math.Abs(Vector3.Dot(face.Plane.Normal, normal)) <= 0.0001)
                {
                    yield return new Problem(GetType(), document, new[] {face}, Fix,
                        "Texture axis perpendicular to face",
                        "The texture axis of this face is perpendicular to the face plane. " +
                        "This occurs when manipulating objects with texture lock off, as well as various other operations. " +
                        "Re-align the texture to the face to repair. Fixing the problem will reset the textures to the face plane."
                    );
                }
            }
        }

...
namespace Sledge.BspEditor.Editing.Commands
{
    ...
    {
        [Import] private CompileSpecificationRegister _register;
        ...
        {
            var spec = _register.GetCompileSpecificationsForEngine(document.Environment.Engine).FirstOrDefault();
            if (spec == null) return;

            using (var cd = new CompileDialog(spec))
            {

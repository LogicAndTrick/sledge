...
namespace Sledge.EditorNew.Environment
{
    public class GameEnvironment
    {
        ...
        public IEnumerable<string> GetGameDirectories()
        {
            ...
            {
                if (!String.IsNullOrWhiteSpace(Game.WonGameDir) && !String.IsNullOrWhiteSpace(Game.ModDir))
                {
                    yield return Path.Combine(Game.WonGameDir, Game.ModDir);
                }
                if (!String.Equals(Game.BaseDir, Game.ModDir, StringComparison.CurrentCultureIgnoreCase)
                    && !String.IsNullOrWhiteSpace(Game.BaseDir) && !String.IsNullOrWhiteSpace(Game.WonGameDir))
                {

...
using Sledge.EditorNew.Extensions;
using Sledge.EditorNew.Rendering.Arrays;
using Sledge.EditorNew.Rendering.Shaders;
...
namespace Sledge.EditorNew.Rendering.Renderers
{
    public class ModernRenderer : IRenderer
    {
        ...
        private Dictionary<IViewport2D, GridArray> GridArrays { get; set; }
        ...
        public ModernRenderer(Document document)
        {
            ...
            GridArrays = ViewportManager.Viewports.OfType<IViewport2D>().Where(x => x.Is2D).ToDictionary(x => x, x => new GridArray());
        ...
        {
            foreach (var vp in ViewportManager.Viewports.OfType<IViewport2D>().Where(x => x.Is2D && !GridArrays.ContainsKey(x)))
            {
        ...
        public void Draw2D(IViewport2D context, Matrix4 viewport, Matrix4 camera, Matrix4 modelView)
        {
        ...
        public void Draw3D(IViewport3D context, Matrix4 viewport, Matrix4 camera, Matrix4 modelView)
        {
            ...
            var opts = new Viewport3DRenderOptions
            {
                ...
                Shaded = type == ViewType.Shaded || type == ViewType.Textured,
                Textured = type == ViewType.Textured,
                Wireframe = type == ViewType.Wireframe
            };

            var cam = context.Camera.Location;

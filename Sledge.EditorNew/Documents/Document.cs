...
using Sledge.EditorNew.Editing;
using Sledge.EditorNew.Environment;
using Sledge.EditorNew.Extensions;
using Sledge.EditorNew.History;
using Sledge.EditorNew.Rendering;
using Sledge.EditorNew.Rendering.Helpers;
...
namespace Sledge.EditorNew.Documents
{
    public class Document : IDocument
    {
        ...
        public string Text { get { return FileName; } }
        ...
        {
            File = System.IO.File.Exists(mapFile) ? new FileInfo(mapFile) : null;
            FileName = String.IsNullOrWhiteSpace(mapFile)
                ? DocumentManager.GetUntitledDocumentName()
                : System.IO.Path.GetFileName(mapFile);
            Map = map;
            Game = game;
            ...
            /* .Union(GameData.MaterialExclusions) */
            // todo material exclusions
            ...
            if (File != null) Mediator.Publish(EditorMediator.FileOpened, File);
        ...
        //    var cam = ViewportManager.Viewports.OfType<IViewport3D>().Select(x => x.Camera).FirstOrDefault();
        //    if (cam != null)
        //    {
        ...
        //public void Make3D(IMapViewport viewport, IViewport3D.ViewType type)
        //{
        ...
        //public void Make2D(IMapViewport viewport, IViewport2D.ViewDirection direction)
        //{

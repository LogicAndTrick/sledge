...
using Sledge.EditorNew.Actions.MapObjects.Groups;
...
using Sledge.EditorNew.Actions.Visgroups;
using Sledge.EditorNew.Clipboard;
using Sledge.EditorNew.Enums;
...
namespace Sledge.EditorNew.Documents
{
    ...
    public class DocumentSubscriptions : IMediatorListener
    {
        ...
        public void Center2DViewsOnSelection()
        {
            var box = _document.Selection.GetSelectionBoundingBox()
                      ?? new Box(Coordinate.Zero, Coordinate.Zero);
            foreach (var vp in ViewportManager.Viewports.Where(x => x.Is2D).OfType<IViewport2D>())
            {
                vp.FocusOn(box);
            }
        }
        ...
        public void Center3DViewsOnSelection()
        {
            var box = _document.Selection.GetSelectionBoundingBox()
                      ?? new Box(Coordinate.Zero, Coordinate.Zero);
            foreach (var vp in ViewportManager.Viewports.Where(x => x.Is3D).OfType<IViewport3D>())
            {
                vp.FocusOn(box);
            }
        }
        ...
        public void ViewportRightClick(IViewport2D vp, ViewportEvent e)
        {
        ...
        public void ViewportCreated(IMapViewport viewport)
        {
            if (viewport.Is3D) viewport.RenderContext.Add(new WidgetLinesRenderable());

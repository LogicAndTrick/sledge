...
{
    /// <summary>
    /// Represents a GCF container or sub-file in the GCF file system
    /// </summary>
    public class GcfFile : IFile
    {
        private FileInfo FileInfo { get; set; }
        private string FilePath { get; set; }

        public GcfFile(string fileName)
        {
            FileInfo = new FileInfo(fileName);
            FilePath = null;
        }

        public GcfFile(string fileName, string path)
        {
            FileInfo = new FileInfo(fileName);
            FilePath = path;
        }
        ...
        public IFile Parent
        {
            get
            {
                return IfContainer<IFile>(
                    () => new NativeFile(FileInfo.Directory),
                    () => new GcfFile(FileInfo.FullName, Path.GetDirectoryName(FilePath)),
                    () => new GcfFile(FileInfo.FullName, Path.GetDirectoryName(FilePath))
                    );
            }
        }
        ...
        public Stream Open()
        {
            // Allow opening the GCF stream, but not a subfolder stream.
            return IfContainer<Stream>(
                () => FileInfo.OpenRead(),
                () => null,
                () => new GcfFilePackageStream(FileInfo.FullName, FilePath)
                );
        }
        ...
        {
            // GCFs do not have related files.
        ...
        internal class GcfFilePackageStream : Stream
        {
            ...
            public GcfFilePackageStream(string gcfPath, string filePath)
            {
                _gcfPath = gcfPath;
                _filePath = filePath;
                Open();
            ...
            private void Open()
            {
                ...
                _open = true;
                HLLib.Initialize();
                _package = new HLLib.Package(_gcfPath);

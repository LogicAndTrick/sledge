...
namespace Sledge.Sandbox
{
    public class MapObject3DShader : IDisposable
    {
        ...
        public void SetLight(int index, Vector3 position, Vector3 ambient, Vector3 diffuse)
        {
            Shader.Set("lights[" + index + "].position", position);
            Shader.Set("lights[" + index + "].ambient", ambient);
            //Shader.Set("lights[" + index + "].diffuse", diffuse);
        }
